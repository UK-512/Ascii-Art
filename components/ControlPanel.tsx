...
import { Sliders, Monitor, Type, Palette, Focus } from 'lucide-react';
...
export const ControlPanel: React.FC<ControlPanelProps> = ({ options, setOptions }) => {
  ...
  return (
    <div className="absolute bottom-0 w-full max-h-[42vh] overflow-y-auto bg-black/80 border-t border-green-900/50 backdrop-blur-sm p-4 z-30 transition-all duration-300">
      <div className="max-w-6xl mx-auto flex flex-wrap gap-6 justify-center items-center text-green-500 text-xs font-mono">
        ...
        {/* Detail / Source Sampling */}
        <div className="flex flex-col gap-1 w-32">
           <div className="flex items-center gap-2 mb-1">
             <Focus className="w-3 h-3" />
             <label>DETAIL: {options.resolution.toFixed(2)}</label>
           </div>
          <input 
            type="range" 
            min="0.10" 
